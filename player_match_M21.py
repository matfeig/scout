...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  3 11:14:17 2023
...
df2 = pd.read_csv("/Users/matfeig/Library/CloudStorage/Dropbox/SFC/Database/player_match/23_24/M21/rapper_sfc_1.csv")
df1 = pd.read_csv("/Users/matfeig/Library/CloudStorage/Dropbox/SFC/Database/event_match/23_24/M21/rapper_sfc_1.csv")
...
desired_positions = ["Center Forward", "Right Wing", "Left Wing", "Left Center Forward", "Right Center Forward","Center Attacking Midfield", "Left Midfield","Right Midfield", "Right Wing Back"]
#desired_positions = ["Left Back"]
servette_players = df1.loc[(df1["team_name"] == "Servette FC M-21") & df1["player_position_name"].isin(desired_positions)]["player_name"].unique()
...
for player_name in servette_players:
    ...
    fig_text(0.07, 0.04, "Red - Ball Lost | " + player_name, color="black", fontweight="bold", fontsize=12, backgroundcolor='white')
    
    # Display the ratio on the top-right corner of the figure
    ax['pitch'].text(0.26, 1, f"Ratio résussi: {ratio:.2%}", 
...
for player in servette_players:
    mask = (df1['player_name'] == player) & (df1['team_name'] == "Servette FC M-21")
    ...
    fig_text(0.07,0.04,f"Noir - Passes | Gris - Carry | Bleu - Dribble - {player}", color = "black",fontweight = "bold", fontsize = 8,backgroundcolor='white')
    ...
    fig.suptitle("Passes Conduites Dribbles", fontsize = 22, fontweight = "bold")
...
team_total_xg = df1[(df1['team_name'] == "Servette FC M-21") & (df1['event_type_name'] == 'Shot')]['statsbomb_xg'].sum()

for player in servette_players:
    mask = (df1['player_name'] == player) & (df1['team_name'] == "Servette FC M-21")
    df_player = df1.loc[mask, :]
    
    shot = df_player.loc[df_player['event_type_name'] == 'Shot'].set_index('id')
    mask_servette = (df_player.event_type_name == 'Shot')
    df_shot = df_player.loc[mask_servette, ['location_x', 'location_y', 'outcome_name', "player_name", "statsbomb_xg"]]

    df_shot["statsbomb_xg"] = np.round(df_shot["statsbomb_xg"], decimals = 3)
    
    total_xg = df_shot["statsbomb_xg"].sum()
    
    player_total_xg = df_shot["statsbomb_xg"].sum()
    player_ratio_xg = player_total_xg / team_total_xg * 100  # as a percentage

#for Servette
    pitch = VerticalPitch(line_color='black',linewidth=1,half=True)
    fig, ax = pitch.grid(grid_height=0.9, title_height=0.06, axis=False,endnote_height=0.04, title_space=0, endnote_space=0)

    for i, row in df_shot.iterrows():
        if row["outcome_name"] == 'Goal':
    #make circle
           pitch.scatter(row.location_x, row.location_y, alpha = 1, s=1000*np.sqrt(row.statsbomb_xg), color = "red", ax=ax['pitch'])
           pitch.annotate(f"{row['statsbomb_xg']*100:.2f}%", (row["location_x"] + 1.7, row["location_y"] - 2), ax=ax['pitch'], fontsize=14)
        else:
            pitch.scatter(row.location_x, row.location_y, alpha = 1, s=1000*np.sqrt(row.statsbomb_xg), color = "black", ax=ax['pitch'])
            pitch.annotate(f"{row['statsbomb_xg']*100:.2f}%", (row["location_x"] + 1.7, row["location_y"] - 2), ax=ax['pitch'], fontsize=14)

    semi_circle = patches.Arc((40, 120), 34, 34, theta1=180, theta2=0, color='black', linestyle=':', linewidth=2)
    ax['pitch'].add_patch(semi_circle)
    
    # Define the semi-circle
    path = patches.PathPatch(
        patches.Path([(40, 120), (60, 120), (40, 120), (40, 120)],
                     [patches.Path.MOVETO, patches.Path.CURVE3, patches.Path.CURVE3, patches.Path.CLOSEPOLY]),
        facecolor='none', edgecolor='black', lw=2)

    ax['pitch'].add_patch(path)

    # Create a radial gradient by overlaying circles with varying color and alpha
    n_circles = 100
    for i in np.linspace(0, 1, n_circles):
        circle = patches.Circle((40, 120), radius=i*18, color=plt.cm.seismic(i), alpha=0.01)
        ax['pitch'].add_patch(circle)


    ax['pitch'].text(0.95, 0.98, f"Total xG: {total_xg:.2f}", color='black', ha='right', va='top', transform=ax['pitch'].transAxes, fontsize=14)
    ax['pitch'].text(0.27, 0.98, f"Team:{team_total_xg:.2f} | Ratio: {player_ratio_xg:.2f}%", color='black', ha='right', va='top', transform=ax['pitch'].transAxes, fontsize=14)


    fig.suptitle("     Shot Map     ", fontsize=30, fontweight="bold", color="black", 
             bbox=dict(facecolor='white', edgecolor='white', boxstyle='round,pad=1'))
    fig_text(0.07,0.04,f"Rouge - buts & Noir - tirs non cadré & Valeur de xG | {player}", color = "black",fontweight = "bold", fontsize = 12,backgroundcolor='white')
    filepath = os.path.join('/Users/matfeig/Documents/SFC/rapport', f'shot_{player}.png')
    fig.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.show()
...
for player in servette_players:
    ...
    player_data = df1.loc[(df1["team_name"] == "Servette FC M-21") & (df1["player_name"] == player) & (~df1['obv_total_net'].isna())]
    ...
    if not player_data.empty:
        ...
        axes.set_title(f'Contribution au jeu - Menace: {sum_obv:.2f} de -4 à +8', fontsize=18, fontweight="bold", color="black", pad=15)
...
player = ["Alexandre Dias Patrício",
          "Tiemoko Ouattara", 
          #"Lois Ndema Mbongué",
          "Rayan Guillaume", 
          #"Rayan Benammar"
...
for player in servette_player:
    player_data = df2.loc[(df2["team_name"] == "Servette FC M-21") & (df2["player_name"] == player)]
    
    waf = player_data[['player_name','player_match_op_passes_into_box','player_match_op_key_passes',
                        'player_match_touches_inside_box','player_match_dribbles','player_match_deep_progressions',
                        'player_match_successful_aerials',"player_match_successful_crosses", "player_match_passes_inside_box"]]
    
    waf = waf.rename(columns={'player_match_op_passes_into_box': 'Passes_into_box','player_match_op_key_passes':'key_passes',
                              'player_match_touches_inside_box':'touches_inside_box','player_match_dribbles':'dribbles',
                              'player_match_deep_progressions':'deep_progressions','player_match_successful_aerials':'aerials' ,'player_match_successful_crosses':'Centres', "player_match_passes_inside_box": "Passe_surface"})
    ...
    op_xa = player_data['player_match_op_xa'].sum()  # assuming you want the sum; adjust as needed
    np_xg = player_data['player_match_np_xg'].sum()  # assuming you want the sum; adjust as needed
    ...
    title_text = f"Apport Offensif | xA: {op_xa:.2f} & xG: {np_xg:.2f}"
    fig.suptitle(title_text, fontsize=11, fontweight="bold", color="black", ha='center')
    #fig_text(0.07, 0.04, f"{player_name}", color="black", fontweight="bold", fontsize=12, backgroundcolor='white')
