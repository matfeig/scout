...
def xT_cumulatif(id_match) :
    ...
    with plt.ioff():
        ...
        plt.fill_between(min,cumul_xt_1, facecolor='#014182',alpha = 0.5)
        plt.fill_between(min,cumul_xt_2, facecolor='#f97306',alpha = 0.5)
        p1 = plt.plot(min,cumul_xt_1,label = team1_name,color = "#014182")
        p2 = plt.plot(min,cumul_xt_2, label = team2_name,color = "#f97306")
        plt.xlabel("Time intervals - every 5 minutes")
        plt.ylabel("Cumulative xT")
        plt.title("Cumulative xT during the match")
...
def xT_diff(id_match) :
    ...
    with plt.ioff():
        ...
        plt.fill_between(min,xt_diff,y0,where = (xt_diff >= y0),interpolate=True,facecolor='#014182',alpha = 0.5)
        plt.fill_between(min,xt_diff,y0,where = (xt_diff <= y0),interpolate=True,facecolor='#f97306',alpha = 0.5)
        plt.plot(min,xt_diff,color = "white")
        plt.plot(time_goals_1,yS,'x',color = "#014182")
        plt.plot(time_goals_2,yO,'x',color = "#f97306")
        plt.xlabel("Time intervals - every 5 minutes")
        plt.ylabel("xT difference")
        plt.title("xT difference during the match")
        ax.set_xticks(time_ind)
        return(fig)
...
def xT_joueur(id_match,team,df_events) :
    ...
    with plt.ioff():
        ...
        plt.xlabel("xT Value", fontsize=15)
        plt.ylabel("Players", fontsize=15)
        plt.title("xT per player", fontsize=20)
