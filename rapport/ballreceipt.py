...
def pass_and_ball_receipt(id_match,team,df_events) :
    ...
    events_pass = df_events_team[(df_events_team.type_name == "Pass")]
    ...
    with plt.ioff():
        ...
        for player in fields_player :
            ...
            pitch = VerticalPitch(line_color='black', line_zorder=2,linewidth=0.3)
            pitch.draw(ax=ax)
            if len(player_ball_receipt) > 6:
                kde = pitch.kdeplot(player_ball_receipt.x, player_ball_receipt.y, ax=ax,
                                    # shade using 100 levels so it looks smooth
                                    shade=True, levels=30,
                                    # shade the lowest area so it looks smooth
                                    # so even if there are no events it gets some color
                                    shade_lowest=True,
                                    cut=25,  # extended the cut so it reaches the bottom edge
                                    cmap=grey_cmap)
            sns.scatterplot(player_ball_receipt.y, player_ball_receipt.x, edgecolor='#870E26', facecolor='#870E26', marker='.', s=8)
            plt.title(nom_de_famille(player) + " (" + str(x_win) +" m)", size=5 )
            i += 1
        #plt.title("Temps de Pressing & Nombre de Pressing",fontsize = 20,fontweight = "bold",backgroundcolor='0.85')
        return(fig)
