...
df_compet = pd.read_json(response.content.decode("utf-8"), convert_dates=['match_updated'])
...
for ind in df_compet.index :
    ...
    df_matches_season = pd.read_json(response.content.decode("utf-8"), convert_dates=['match_date', 'last_updated'])
