...
def xT_joueur(id_match,team,df_events) :
    ...
    with plt.ioff():
        ...
        ax = plt.axes()
        b1 = plt.barh(range(n), y_xT_pos, height=0.7, color="green", alpha=0.8)
        b2 = plt.barh(range(n), y_xT_neg, height=0.7, color="red", alpha=0.8)
        plt.axvline(0.6, 0.05, 0.95,linewidth=1, color = "black")
        plt.axvline(0.4, 0.05, 0.95,linewidth=1, color = "black")
        fig_text(0.63,0.46,"Menacant", color = "black",fontweight = "bold", fontsize = 15,backgroundcolor='0.85')
        fig_text(0.78,0.46,"Très Menacant", color = "black",fontweight = "bold", fontsize = 15,backgroundcolor='0.85')
        plt.barh(range(n), y_xT, height=0.2, color="white", alpha=0.8)
        plt.xlim([-0.3, 0.9])
        plt.xlabel("Valeurs de Menace", fontsize=20,fontweight = "bold", labelpad=15)
        #plt.ylabel("Joueurs", fontsize=20, fontweight = "bold")
        plt.title("Créateurs de Menace", fontsize=30,fontweight = "bold", pad=25)
        plt.yticks(range(n), x_name)
        plt.legend([b1,b2],["Positif","Negatif"],frameon = True,loc = 'lower right')
        return(fig)
...
#         ax = plt.axes()
#         plt.barh(range(n), y_xT_pos, height=0.6, color="#009900", alpha=0.8)
#         plt.barh(range(n), y_xT_neg, height=0.6, color="#FF0000", alpha=0.8)
#         plt.barh(range(n), y_xT, height=0.2, color="white", alpha=0.8)
#         plt.xlim([-0.5, 0.9])
#         plt.xlabel("Vert => Positif | Rouge = Négatif", fontsize=18)
#         plt.ylabel("Joueurs", fontsize=18)
#         plt.title("Créateur de danger", fontsize=20)
#         plt.yticks(range(n), x_name)
#         return(fig)
